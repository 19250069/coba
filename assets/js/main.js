...
(function () {
    ...
    function applyDark(mode) {
        ...
        if (t) t.innerHTML = mode === 'dark' ? '<i class="fas fa-sun"></i>' : '<i class="fas fa-moon"></i>';
    }
    ...
    if (searchInput) {
        ...
        function doSearch(){
            ...
            timer = setTimeout(function () {
                ...
                fetch(BASE + 'api/obat_search.php?q=' + encodeURIComponent(q) + '&kategori=' + encodeURIComponent(kat) + '&harga_min=' + encodeURIComponent(hmin) + '&harga_max=' + encodeURIComponent(hmax))
    ...
    /* ---------- KERANJANG AJAX ---------- */
    window.addToCart = function (id_obat) {
        postData(BASE + 'api/keranjang.php', { act: 'add', id_obat: id_obat, csrf_token: window.CSRF })
            .then(res => {
