...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        $insD = $pdo->prepare("INSERT INTO detail_pesanan (id_pesanan, id_obat, harga, jumlah, subtotal) VALUES (?,?,?,?,?)");
        $upStok = $pdo->prepare("UPDATE obat SET stok = stok - ? WHERE id_obat=? AND stok >= ?");
        foreach ($items as $it) {
            $sub = $it['harga'] * $it['jumlah'];
            $insD->execute([$id_pesanan, $it['id_obat'], $it['harga'], $it['jumlah'], $sub]);
            $upStok->execute([$it['jumlah'], $it['id_obat'], $it['jumlah']]);
        }
        ...
        $pdo->prepare("INSERT INTO pembayaran (id_pesanan, id_metode, metode, status) VALUES (?,?,?,'Menunggu')")->execute([$id_pesanan, $metode_id, $metode_nama]);
        ...
        send_notif($pdo, $uid, 'Pesanan Dibuat', "Pesanan $kode berhasil dibuat, silakan bayar.");
            ...
            <div class="card-body">
                <form method="post" id="formCheckout">
                    ...
                    <div class="alert alert-info small">
                        <i class="fas fa-info-circle me-1"></i> Setelah checkout, Anda akan diarahkan ke halaman upload bukti transfer.
                    </div>
                </form>
            </div>
        </div>
