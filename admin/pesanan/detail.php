...
<div class="row g-3">
    ...
    <div class="col-lg-4">
        ...
        <div class="card">
            ...
            <div class="card-body">
                <p class="mb-1">Metode: <strong><?php if (!empty($pay['id_metode'])): $d=$pdo->prepare("SELECT nama_metode,nomor,pemilik,tipe FROM metode_pembayaran WHERE id_metode=?"); $d->execute([$pay['id_metode']]); $d=$d->fetch(); if ($d): ?><?= e($d['nama_metode'] ?? '-') ?><?php else: ?><?= e($pay['metode'] ?? '-') ?><?php endif; ?><?php else: ?><?= e($pay['metode'] ?? '-') ?><?php endif; ?></strong></p>
                <p class="mb-1">Tujuan: <span><?php if (!empty($pay['id_metode']) && $d): ?><?= e($d['tipe']==='bank'?'Rekening Bank':'E-Wallet') ?> <?= e($d['nomor']) ?> a.n. <?= e($d['pemilik']) ?><?php else: ?>-<?php endif; ?></span></p>
                <p class="mb-1">Status: <?= status_badge($pay['status']) ?></p>
                <?php if ($pay['bukti_transfer']): ?><img src="<?= URL_BUKTI . e($pay['bukti_transfer']) ?>" class="img-fluid rounded" onerror="this.src='<?= BASE_URL ?>assets/images/no-image.svg'"><p class="small mt-1"><?= e($pay['bukti_transfer']) ?></p><?php endif; ?>
            </div>
        </div>
        <?php endif; ?>
        <div class="card">
