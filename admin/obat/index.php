...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($act === 'tambah') {
        ...
        $pdo->prepare("INSERT INTO obat (kode_obat,nama_obat,id_kategori,id_supplier,harga,stok,deskripsi,gambar,tanggal_expired,created_at) VALUES (?,?,?,?,?,?,?,?,?,NOW())")
            ->execute([$kode, $nama, $id_kat, $id_sup, $harga, $stok, $desc, $gambar, $exp ?: null]);
        set_flash('success', 'Obat berhasil ditambahkan.');
    } elseif ($act === 'edit') {
        $id = (int)$_POST['id'];
        if ($gambar === null) {
            $pdo->prepare("UPDATE obat SET nama_obat=?,id_kategori=?,id_supplier=?,harga=?,stok=?,deskripsi=?,tanggal_expired=? WHERE id_obat=?")
                ->execute([$nama, $id_kat, $id_sup, $harga, $stok, $desc, $exp ?: null, $id]);
        } else {
            ...
            $pdo->prepare("UPDATE obat SET nama_obat=?,id_kategori=?,id_supplier=?,harga=?,stok=?,deskripsi=?,gambar=?,tanggal_expired=? WHERE id_obat=?")
                ->execute([$nama, $id_kat, $id_sup, $harga, $stok, $desc, $gambar, $exp ?: null, $id]);
        }
