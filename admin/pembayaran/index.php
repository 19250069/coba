...
$pays = $pdo->query("SELECT pb.*, p.kode_pesanan, p.total, u.nama FROM pembayaran pb JOIN pesanan p ON p.id_pesanan=pb.id_pesanan JOIN users u ON u.id_user=p.id_user ORDER BY pb.id_pembayaran DESC")->fetchAll();
        ...
        <table id="table" class="table table-hover" style="width:100%">
            ...
            <?php foreach ($pays as $pb): ?>
                <tr>
                    ...
                    <td><?= e($pb['nama']) ?></td>
                    <td><?php if (!empty($pb['id_metode'])): $d=$pdo->prepare("SELECT nama_metode FROM metode_pembayaran WHERE id_metode=?"); $d->execute([$pb['id_metode']]); $d=$d->fetch(); if ($d): ?><?= e($d['nama_metode'] ?? '-') ?><?php else: ?><?= e($pb['metode'] ?? '-') ?><?php endif; ?><?php else: ?><?= e($pb['metode'] ?? '-') ?><?php endif; ?></td>
                    <td><?php if (!empty($pb['id_metode'])): $d=$pdo->prepare("SELECT nomor,pemilik FROM metode_pembayaran WHERE id_metode=?"); $d->execute([$pb['id_metode']]); $d=$d->fetch(); if ($d): ?><?= e($d['nomor'] ?? '') ?> a.n. <?= e($d['pemilik'] ?? '') ?><?php endif; ?><?php endif; ?></td>
