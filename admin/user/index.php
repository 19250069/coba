...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($act === 'tambah') {
        ...
        $pdo->prepare("INSERT INTO users (nama,email,password,no_hp,alamat,role,created_at) VALUES (?,?,?,?,?,?,NOW())")->execute([$nama, $email, $pass, $hp, $alamat, $role]);
        ...
        if (!empty($_POST['password'])) {
            $pdo->prepare("UPDATE users SET nama=?,email=?,password=?,no_hp=?,alamat=?,role=? WHERE id_user=?")
                ->execute([$nama, $email, password_hash($_POST['password'], PASSWORD_DEFAULT), $hp, $alamat, $role, $id]);
        } else {
            $pdo->prepare("UPDATE users SET nama=?,email=?,no_hp=?,alamat=?,role=? WHERE id_user=?")
                ->execute([$nama, $email, $hp, $alamat, $role, $id]);
        }
...
<div class="card">
    ...
    <div class="card-body">
        <table id="table" class="table table-hover" style="width:100%">
            <thead><tr><th>#</th><th>Nama</th><th>Email</th><th>No. HP</th><th>Role</th><th class="text-end">Aksi</th></tr></thead>
            <tbody>
                ...
                <tr>
                    ...
                    <td class="text-end">
                        <button class="btn btn-sm btn-info" data-bs-toggle="modal" data-bs-target="#modalDetail"
                            data-nama="<?= e($u['nama']) ?>" data-email="<?= e($u['email']) ?>" data-hp="<?= e($u['no_hp']) ?>" data-alamat="<?= e($u['alamat']) ?>" data-role="<?= e($u['role']) ?>" data-created="<?= e($u['created_at']) ?>"><i class="fas fa-eye"></i></button>
                        <button class="btn btn-sm btn-warning" data-bs-toggle="modal" data-bs-target="#modalForm"
                            data-id="<?= $u['id_user'] ?>" data-nama="<?= e($u['nama']) ?>" data-email="<?= e($u['email']) ?>" data-hp="<?= e($u['no_hp']) ?>" data-alamat="<?= e($u['alamat']) ?>" data-role="<?= e($u['role']) ?>"><i class="fas fa-edit"></i></button>
...
$("#modalForm").on("show.bs.modal", function(e){
    ...
    if (b.data("id")) { $("#fAct").val("edit"); $("#fId").val(b.data("id")); $("#fNama").val(b.data("nama")); $("#fEmail").val(b.data("email")); $("#fHp").val(b.data("hp")); $("#fAlamat").val(b.data("alamat")); $("#fRole").val(b.data("role")); $(this).find(".modal-title").text("Edit User"); }
    else { $("#fAct").val("tambah"); $("#fId").val(""); $("#fNama").val(""); $("#fEmail").val(""); $("#fHp").val(""); $("#fAlamat").val(""); $("#fRole").val("customer"); $(this).find(".modal-title").text("Tambah User"); }
});
$("#modalDetail").on("show.bs.modal", function(e){
    var b = $(e.relatedTarget);
    $("#dNama").text(b.data("nama")); $("#dEmail").text(b.data("email")); $("#dHp").text(b.data("hp")); $("#dRole").text(b.data("role")); $("#dAlamat").text(b.data("alamat")); $("#dCreated").text(b.data("created"));
});
</script>';
